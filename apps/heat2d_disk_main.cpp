// ─────────────────────────────────────────────────────────────────────────────
// heat2d_disk_main.cpp — 2D Heat Diffusion + sürüklenebilir sıcak disk
//
// Disk içi her adımda T=1.0 (Dirichlet sıcak kaynak)
...
int main(int argc, char* argv[]) {
    const char* cfg_path = (argc > 1) ? argv[1] : "heat2d_disk.cfg";
    ...
    auto apply_bc = [&](std::vector<double>& T) {
        ...
        // Disk: Dirichlet T=1
        int rc = (int)std::ceil(disk.r);
        int ci = (int)std::round(disk.cy);
        int cj = (int)std::round(disk.cx);
        for (int di = -rc; di <= rc; ++di)
            for (int dj = -rc; dj <= rc; ++dj) {
                int gi = ci+di, gj = cj+dj;
                if (gi<0||gi>=ny||gj<0||gj>=nx) continue;
                if (disk.contains(gi, gj)) T[grid.idx(gj, gi)] = 1.0;
            }
    };
    ...
    if (SDL_Init(0) == 0) {} // already done by renderer

    while (renderer.is_open()) {
        // Manual event loop (need mouse events SDL2Renderer doesn't expose)
        SDL_Event e;
